...
def test_context_isolation():
    """人格混線対策: 自分=assistant / 他者=【名前】付き user になっているか。"""
...
def test_no_silence_and_round_robin():
    """沈黙対策: 全パネリストが各ラウンドで必ず1回発言し、書記は喋らない。"""
...
def test_streaming():
    ...
    for turn in council.run("議題T", emit=events.append):
        # API 層を模して、Turn 確定後に turn_end を出す
...
def test_session_transport():
    ...
    # turn_start→delta*→turn_end が含まれる（イベント列が乗っている）
...
def test_followup_injection():
    """pull で追い質問を注入。人間ターン→司会再提示→パネリスト1周。
...
def test_llm_status():
    """llm_status は環境で切替・キー値を漏らさない／mock=True は常に Mock。"""
...
# ===========================================================================
# 回帰テスト群（research/artifact 周りの修正）。実LLM/実SDKは使わず注入Fakeで検証する。
# ===========================================================================
...
def test_strip_model_artifacts():
    """tool-call 特殊トークン・<think> を全文確定地点で除去（通常文は不変）。"""
    print("[test] strip model artifacts: タグ/特殊トークン除去")
    ...
    # 回帰: AI内部仕様を“話題として”論じる討論本文を全消ししない。
    ...
    # 回帰の番人: 可視本文が artifact だけのストリームは「artifact 除去後の空」で救済が発火する。
...
def test_openai_retry_truncation_marker():
    """空ターン救済リトライ後も length を検出し途中切れ目印を付ける（空retryはゴースト防止）。"""
    print("[test] openai retry truncation marker: 救済後の途中切れ検出")
...
def test_openrouter_search_nonjson():
    """urlopen 戻りが非JSONでも例外で死なず失敗文言＋秘密伏字、stream:false/Accept を要求。"""
    print("[test] openrouter search non-JSON robustness")
...
def test_opening_directive_no_nomination():
    """司会オープニングが個人名指しでなく『全員が順に』告知（RoundRobin 実順と一致）。"""
    print("[test] opening directive no nomination: 司会指名の食い違い解消")
    coun = Council(
    ...
    check("特定の立場の人へ発言を促す" not in alltext, "『特定の立場の人へ発言を促す』文言が出ない（個人名指しでなく全員順次の告知）")
    check(
...
def test_research_nudge_phase_gating():
    """research=True でも枠組みフェーズ（redefine/bridge/closing/収束口火）に要調査を出さない。"""
    print("[test] research nudge phase gating: 捉え直し等で要調査を誘発しない")
    from core.context import RESEARCH_NUDGE
...
def test_research_sources_truncated_in_context():
    """調査ターンの出典は LLM 文脈では上位N件に圧縮（Turn.content 自体はフル保持）。"""
    print("[test] research sources truncated in context: 文脈肥大の抑制")
    ...
    # 回帰: 出典の途中に空行が混入しても全件を1ブロックとして掴み圧縮する。
...
def test_research_prompt_and_format():
    """研究プロンプトに分量ガード、出典整形は共通ヘルパに集約（出力はバイト一致）。"""
    print("[test] research prompt guard + format helper")
...
def test_research_useful_and_cap_constants():
    """低価値ブリーフ判定と定型文言のバイト一致（cap を有益調査だけで消費）。"""
    print("[test] research useful filter + low-value prefixes")
