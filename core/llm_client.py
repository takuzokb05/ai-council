...
1プロセスで複数プロバイダーを同時に吸収すると不安定になりがちなので、IF を1つに統一する。
...
logger = logging.getLogger("ai_council.llm")
...
# （AI内部仕様を論じる討論での発言全消失を回避）。途中の孤立タグは _ORPHAN_TAG_RE
# が「タグだけ」落とし本文は残す。
...
class OpenAIClient(LLMClient):
    ...
    1プロセスで複数 provider を同時に吸収すると不安定になるため、それはしない。1セッション=1 provider=
