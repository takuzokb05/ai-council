...
ストリーミングのライブ感（1発言ずつ流れる様子）を Mock で見たいときは `.env` に `AI_COUNCIL_MOCK_DELAY=0.2` を入れる。
...
- **共有/公開運用（BYOK）**: 環境変数 `AI_COUNCIL_BYOK=1` を立てると、**サーバのキーを来訪者に一切使わせない**。
  来訪者は自分のキーを画面で入力し（ブラウザの localStorage のみに保存・サーバ非保存）、毎リクエストのヘッダで送る。
  キー未入力なら自動で Mock。あわせて `AI_COUNCIL_READONLY=1`（編成 CRUD を 403）とレート制限の利用を推奨。

> 実 LLM を使うと API 利用料が発生する。討論はサーバ側のバックグラウンド実行（“プロデューサ”）で進むため、
> 開始後にブラウザを閉じても最後まで走りコストが出る（止めるには停止/終了操作が要る）点に注意。

主な環境変数は `.env.example` を参照。環境変数は `AI_COUNCIL_` プレフィックスを使う（`AI_COUNCIL_BYOK` 等）。後方互換のため旧 `AI_TEAMS_` プレフィックスも読む（`AI_COUNCIL_` が優先）。
